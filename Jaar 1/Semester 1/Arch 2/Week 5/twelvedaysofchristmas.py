...
def get_suffix(number: int) -> str:
    return f'{number }{suffix.get(number, "th")}'


def next_verse(verse_number: int) -> str:

    next_verses = verses[:verse_number]

    next_verses.reverse()

    last_verse = next_verses.pop()

    return_verse = (
        f"On the {get_suffix(verse_number)} day of Christmas, my true love sent to me "
    )

    if verse_number == 1:
        return return_verse + f"{last_verse}"

    return return_verse + f'{",".join(next_verses)} And {last_verse}'


def main():

    for verse_index in range(1, len(verses) + 1):
        print(next_verse(verse_index))
